...
void send_log_via_bluetooth(char* message)
{
    ...
    char log_flag[50] = "L:";
    bytes_to_send = sprintf(nus_string, "%s%s", log_flag, message);
...
void send_csr_via_bluetooth(char* csr) 
{
    ret_code_t err_code; 
    uint8_t nus_string[2000]; 
    uint16_t bytes_to_send; 

    char csr_message[50] = "C:";
    bytes_to_send = sprintf(nus_string, "%s%s", csr_message, csr);  
    err_code = ble_nus_data_send(&m_nus, nus_string, &bytes_to_send, m_conn_handle);
    printf("DATA SENT VIA BLE:::%s\n\r", nus_string);
}
